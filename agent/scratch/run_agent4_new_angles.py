"""用升级后的 Agent4 为 3 个房源各写 2 个新角度（纠结对比/求建议、意外种草），共 6 篇笔记。"""
import sys
import json
import random
...
RUN_ID = "20260615_1112"
ANGLE_ORDER = ["纠结对比", "意外种草"]
...
def main():
    ...
    print(f"共 {len(targets)} 个房源，每个 {len(ANGLE_ORDER)} 个新角度，共 {len(targets) * len(ANGLE_ORDER)} 篇")
