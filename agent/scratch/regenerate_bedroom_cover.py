...
# ═══════════════════════════════════════════════════════════════
# Gemini Image Gen call with retry loop
# ═══════════════════════════════════════════════════════════════
...
def custom_build_cover_prompt(property_data, room_type="bedroom"):
    ...
    prompt = (
        ...
        f"2. Do NOT widen, expand, or stretch the room. The room boundaries, walls, ceilings, floors, and dimensions stay 100% identical to the input image.\n"
        f"3. {staging_guide}\n"
        f"4. Keep the window area completely clear. Do not place any furniture, thick curtains, plants, or tall items blocking the window view or daylight.\n\n"
        f"HARD RULES:\n"
        f"- 🚫 RED LINE — ABSOLUTELY DO NOT ENLARGE THE ROOM. The square footage, floor area, room dimensions, and ceiling height are FIXED and SACRED. You may slightly adjust camera angle/perspective, but the room MUST remain exactly the same size as in the original photo. If the original only shows one corner of the room, you stay in that corner — do NOT invent space beyond what's visible. Pushing walls back, expanding floor, raising ceilings, or making the space look bigger than it actually is = INSTANT FAILURE.\n"
        f"- If the original photo only shows part of the room, present that part beautifully — do NOT invent or imagine extra spaces, rooms, or balconies.\n"
        ...
        f"- Keep original wall colors, flooring, windows, and room layout. No fake windows. No moving walls. Use polished white marble details for accents (如大理石床头柜台面/局部大理石纹) and add subtle luxury staging textures.\n"
    )
    return prompt
...
print("Calling Gemini Image Gen API...")
...
print("Bedroom cover regeneration completed successfully!")
