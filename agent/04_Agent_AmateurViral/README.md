...
**支持两种模式**：
- **房源多角度模式**：同一套房源 × 12种写作角度，分三大类——忍痛放弃类（放弃叙事/意外种草/纠结对比/决赛圈对比/住得下吗/召唤过来人）｜长什么样类（优缺点总结/看房日记/看房踩坑/长啥样开箱/单价你会买吗）｜询问类（仅预算现实吗）
...
| 角度 | Key | 基调 | 是否提小姐姐 |
|------|-----|------|-------------|
| 预算现实吗 | `"预算现实吗"` | **需求口吻**（非看房笔记）：标题「X万+在X地+买X房」固定、结尾反问换花样；正文很短，只说自己想要的(预算/地点/房型+笼统偏好)，禁止"去看了某盘"和精确参数 | 不需要 |
...
把一个 run 的笔记打包成「**发布版**」：不按大类分文件夹，每篇一个以**角度编号打头**的文件夹，
内含 `<标题>.md`（文案，无"标题/正文"标签）+ `<标题>.png`（大字报封面，如有）。

> ### ⚠️ 红线：仅在用户明确要求时才导出发布版
> 这个能力**默认不自动执行**。生成完笔记/配完封面**不要顺手导出发布版**，
> 只有用户明确说"导出/生成发布版"时才运行 `publish_export`。

**编号体系**（大类 A/B/C，细分与角度编号一致）：
`A1放弃叙事 A2意外种草 A3纠结对比 A4决赛圈对比 A5住得下吗 A6召唤过来人` /
`B1优缺点总结 B2看房日记 B3看房踩坑 B4长啥样开箱 B5单价你会买吗` / `C1预算现实吗`

**用法**：
```bash
python -m 04_Agent_AmateurViral.publish_export \
  --run-dir "/path/to/<run_id>" \
  --out "/path/发布版"          # 默认 <run-dir>/发布版
  # --include-photos            # 可选：同时复制房图 photo_2/3（默认只 md+封面）
```
- 角度识别顺序：note 文件夹的 `_angle.txt`（新 run 自动写入）→ `drafts/*.json` 的 `_category` → 关键词兜底
- 产物结构：`发布版/A2_<标题>/<标题>.md + <标题>.png`，按编号排序时同视角自动聚在一起
- 顺手规范笔记里的标签行为 `#t1 #t2`
