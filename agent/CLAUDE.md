...
Agent6 生成的封面是 AI 装修效果图，不是房屋现状实拍。每篇笔记正文第一段**必须**是这句固定声明：

> 封面图片为装修效果参考图，仅供展示装修后的空间效果；房屋现状实拍请参见笔记【第二页】！！

这句话由 `convert_to_markdown()` 里的 `COVER_DISCLAIMER` 常量强制拼接，不依赖 LLM 自己写
（避免措辞跑偏/遗漏）；`_validate_note()` 会检测正文是否重复写了这句话，重复则判校验失败触发重试。
**禁止手写文案时漏掉这句声明，也禁止改写它的措辞**——原因和其他红线一样：这是合规/防误导要求，
必须保持文字精确、可重复，不随执行者是谁而漂移。
