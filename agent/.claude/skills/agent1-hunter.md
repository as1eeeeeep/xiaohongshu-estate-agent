...
- 获取评论超时 120 秒，下载超时 120 秒（`DOWNLOAD_NOTE_TIMEOUT = 120`）
- 不在短时间内大批量连续请求同一接口
...
除了通用的 `python -m 01_Agent_Scraper.hunter`，目录下有三个预配置的一次性运行脚本：

| 脚本 | 用途 | 时效窗口 | 视角 | 目标数量 |
|---|---|---|---|---|
| `run_amateur_buyhouse.py` | 抓素人买房/入伙/实拍类笔记 | 不限时间 | 素人 | 100 条 |
| `run_agent_buyhouse.py` | 抓中介视角的香港买房笔记 | 近 30 天 | 中介 | 80 条 |
| `run_amateur_2months.py` | 抓素人看房/买房日记 | 近 60 天 | 素人 | 80 条 |

这三个脚本通过覆写 `hunter.PUBLISHED_WITHIN_DAYS` 并传入专题关键词来实现定制搜索，共享 `hunter.py` 的全部逻辑和输出路径。
